...
class InvalidParameterError {
    constructor(smartContractMethodName, sigs, args) {
        ...
        this.message = `${smartContractMethodName} Received ${args.length} arguments while expecting one of the follow signatures (${JSON.stringify(sigs)})`;
    }
}
...
export class ContractMethod {
    ...
    toTransferParams({ fee, gasLimit, storageLimit, amount = 0, mutez = false, } = {}) {
        return {
            to: this.address,
            amount,
            fee,
