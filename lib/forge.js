 ...
 */
const bool = (boolArg) => (boolArg ? 'ff' : '00');
/**
 ...
 */
const address = (addressArg, protocol = '') => {
    const fa = [];
 ...
 * @returns {string} Forged operation bytes
 */
const seedNonceRevelation = (opArg) => {
    const fop = [];
    const levelBuffer = toBuffer(toBytesInt32(opArg.level));
 ...
 * @returns {string} Forged operation bytes
 */
const doubleEndorsementEvidence = (opArg) => {
 ...
 * @returns {string} Forged operation bytes
 */
const doubleBakingEvidence = (opArg) => {
 ...
 * @returns {string} Forged operation bytes
 */
const transaction = (opArg, protocol) => {
    const fop = [];
    fop.push(address(opArg.source, protocol));
    fop.push(zarith(opArg.fee));
    fop.push(zarith(opArg.counter));
 ...
 * @returns {string} Forged operation bytes
 */
const origination = (opArg, protocol) => {
 ...
 * @returns {string} Forged operation bytes
 */
const delegation = (opArg, protocol) => {
    const fop = [];
    fop.push(address(opArg.source, protocol));
    fop.push(zarith(opArg.fee));
    fop.push(zarith(opArg.counter));
